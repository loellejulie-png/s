...
import { OpenConnectService } from '../services/OpenConnectService.js';
...
export class SquareGolfApp {
    constructor() {
        ...
        this.openConnectService = new OpenConnectService(this.api, this.eventBus);
    ...
    setupEventBusListeners() {
        ...
        this.eventBus.on('device:armed', () => {
            this.toast.success('Ball detection armed');
        });
        ...
        // OpenConnect events
        this.eventBus.on('openconnect:connecting', () => {
            this.toast.info('OpenConnect connection initiated...');
        });
        this.eventBus.on('openconnect:disconnecting', () => {
            this.toast.info('OpenConnect disconnection initiated...');
        });
        this.eventBus.on('openconnect:error', (msg) => this.toast.error(`OpenConnect: ${msg}`));
        this.eventBus.on('openconnect:status', (status) => this.updateOpenConnectStatus(status));
    ...
    setupEventListeners() {
        ...
        document.getElementById('statusOpenConnect')?.addEventListener('click', () => this.screen.show('openConnect'));
        ...
        document.getElementById('gsproArmBtn')?.addEventListener('click', () => {
            this.deviceService.armBallDetection();
        });
        ...
        // OpenConnect controls
        document.getElementById('openConnectConnectBtn')?.addEventListener('click', () => {
            const ip = document.getElementById('openConnectIP').value.trim();
            const port = parseInt(document.getElementById('openConnectPort').value);
            this.openConnectService.connect(ip, port);
        });
        document.getElementById('openConnectDisconnectBtn')?.addEventListener('click', () => {
            this.openConnectService.disconnect();
        });

        // OpenConnect settings
        document.getElementById('openConnectIP')?.addEventListener('change', () => this.saveOpenConnectConfig());
        document.getElementById('openConnectPort')?.addEventListener('change', () => this.saveOpenConnectConfig());
        document.getElementById('openConnectAutoConnect')?.addEventListener('change', () => this.saveOpenConnectConfig());
    ...
    handleWebSocketMessage(message) {
        switch (message.type) {
            ...
            case 'infiniteTeesStatus':
                ...
                break;
            case 'openConnectStatus':
                this.openConnectService.updateStatus(message.data);
                break;
            case 'cameraConfig':
    ...
    updateDeviceStatus(status) {
        ...
        const armBtn = document.getElementById('gsproArmBtn');
    ...
    updateOpenConnectStatus(status) {
        // Update the global status bar OC indicator
        const statusOC = document.getElementById('statusOpenConnect');
        if (statusOC) {
            if (status.connectionStatus === 'connected') {
                statusOC.classList.add('connected');
                statusOC.classList.remove('disconnected');
            } else {
                statusOC.classList.remove('connected');
                statusOC.classList.add('disconnected');
            }
        }

        const statusElement = document.getElementById('openConnectStatus');
        const errorElement = document.getElementById('openConnectError');
        const connectBtn = document.getElementById('openConnectConnectBtn');
        const disconnectBtn = document.getElementById('openConnectDisconnectBtn');
        const ipField = document.getElementById('openConnectIP');
        const portField = document.getElementById('openConnectPort');
    ...
    async saveOpenConnectConfig() {
        const ip = document.getElementById('openConnectIP')?.value.trim();
        const port = parseInt(document.getElementById('openConnectPort')?.value);
        const autoConnect = document.getElementById('openConnectAutoConnect')?.checked;

        await this.openConnectService.saveConfig(ip, port, autoConnect);
    }
    ...
    applySettings(settings) {
        ...
        const handednessRadio = document.querySelector(`input[name="handedness"][value="${handedness}"]`);
        ...
        const ocIP = document.getElementById('openConnectIP');
        const ocPort = document.getElementById('openConnectPort');
        const ocAutoConnect = document.getElementById('openConnectAutoConnect');
        if (ocIP) ocIP.value = settings.openConnectIP || '127.0.0.1';
        if (ocPort) ocPort.value = settings.openConnectPort || 922;
        if (ocAutoConnect) ocAutoConnect.checked = settings.openConnectAutoConnect || false;
    }
