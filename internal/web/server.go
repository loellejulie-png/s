...
import (
	...
	"github.com/brentyates/squaregolf-connector/internal/core/openconnect"
...
type Server struct {
	...
	openConnectIntegration   *openconnect.Integration
...
func NewServer(stateManager *core.StateManager, bluetoothManager *core.BluetoothManager, launchMonitor *core.LaunchMonitor, cameraManager *camera.Manager, gsproIP string, gsproPort int, itIP string, itPort int, ocIP string, ocPort int, enableExternalCamera bool) *Server {
	...
	ocIntegration := openconnect.GetInstance(stateManager, launchMonitor, ocIP, ocPort)
	...
	server := &Server{
		...
		openConnectIntegration:  ocIntegration,
...
func (s *Server) setupCallbacks() {
	...
	s.stateManager.RegisterOpenConnectStatusCallback(func(oldValue, newValue core.OpenConnectConnectionStatus) {
		s.broadcastOpenConnectStatus()
	})
...
func (s *Server) broadcastOpenConnectStatus() {
	status := s.getOpenConnectStatus()
	msg := WSMessage{Type: "openConnectStatus", Data: status}
	data, _ := json.Marshal(msg)
	select {
	case s.broadcast <- data:
	default:
...
func (s *Server) getOpenConnectStatus() OpenConnectStatus {
	var lastErrorStr string
	if err := s.stateManager.GetOpenConnectError(); err != nil {
		lastErrorStr = err.Error()
	}

	connectionStatus := "disconnected"
	switch s.stateManager.GetOpenConnectStatus() {
	case core.OpenConnectStatusConnected:
		connectionStatus = "connected"
	case core.OpenConnectStatusConnecting:
		connectionStatus = "connecting"
	case core.OpenConnectStatusError:
		connectionStatus = "error"
	}

	ip, port := s.openConnectIntegration.GetConnectionInfo()
	settings := config.GetInstance().GetSettings()

	return OpenConnectStatus{
		ConnectionStatus: connectionStatus,
		IP:               ip,
		Port:             port,
		AutoConnect:      settings.OpenConnectAutoConnect,
...
func (s *Server) Start(port int) error {
	...
	// OpenConnect endpoints
	api.HandleFunc("/openconnect/status", s.handleOpenConnectStatus).Methods("GET")
	api.HandleFunc("/openconnect/connect", s.handleOpenConnectConnect).Methods("POST")
	api.HandleFunc("/openconnect/disconnect", s.handleOpenConnectDisconnect).Methods("POST")
	api.HandleFunc("/openconnect/config", s.handleOpenConnectConfig).Methods("GET", "POST")
...
func (s *Server) sendInitialStatus(conn *websocket.Conn) {
	...
	// Send OpenConnect status
	ocStatus := s.getOpenConnectStatus()
	msg = WSMessage{Type: "openConnectStatus", Data: ocStatus}
	data, _ = json.Marshal(msg)
	conn.WriteMessage(websocket.TextMessage, data)
...
func (s *Server) handleOpenConnectStatus(w http.ResponseWriter, r *http.Request) {
	status := s.getOpenConnectStatus()
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(status)
}

func (s *Server) handleOpenConnectConnect(w http.ResponseWriter, r *http.Request) {
	var req struct {
		IP   string `json:"ip"`
		Port int    `json:"port"`
	}
	...
	go func() {
		s.openConnectIntegration.ResetReconnectionState()
		s.openConnectIntegration.EnableAutoReconnect()
		s.openConnectIntegration.Start()
		s.openConnectIntegration.Connect(req.IP, req.Port)
	}()
	w.WriteHeader(http.StatusOK)
}

func (s *Server) handleOpenConnectDisconnect(w http.ResponseWriter, r *http.Request) {
	go func() {
		s.openConnectIntegration.DisableAutoReconnect()
		s.openConnectIntegration.Disconnect()
	}()
	w.WriteHeader(http.StatusOK)
}

func (s *Server) handleOpenConnectConfig(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		settings := config.GetInstance().GetSettings()
		configData := struct {
			IP          string `json:"ip"`
			...
			IP:          settings.OpenConnectIP,
			Port:        settings.OpenConnectPort,
			AutoConnect: settings.OpenConnectAutoConnect,
		}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(configData)
		...
		cfg := config.GetInstance()
		cfg.SetOpenConnectIP(configData.IP)
		cfg.SetOpenConnectPort(configData.Port)
		cfg.SetOpenConnectAutoConnect(configData.AutoConnect)
...
func (s *Server) handleSettings(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		cfg.SetOpenConnectIP(appSettings.OpenConnectIP)
		cfg.SetOpenConnectPort(appSettings.OpenConnectPort)
		cfg.SetOpenConnectAutoConnect(appSettings.OpenConnectAutoConnect)
		...
		// Apply handedness to state
		var handedness core.HandednessType
		if appSettings.Handedness == "left" {
...
func (s *Server) handleAlignmentHandedness(w http.ResponseWriter, r *http.Request) {
	...
	// Persist to config
	config.GetInstance().SetHandedness(req.Handedness)
...
func (s *Server) GetOpenConnectIntegration() *openconnect.Integration {
	return s.openConnectIntegration
}
